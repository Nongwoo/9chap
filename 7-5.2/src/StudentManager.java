...
public class StudentManager {
	...
	private void read() {
		...
		for (int i=0; i<4; i++) {
			...
			dept.put(name, student);  } } //해시맵(name, student)  				
	private void printAll() {
		Set<String> key = dept.keySet(); //모든 값 검색
		Iterator<String> it = key.iterator();
		while (it.hasNext()) { 
			String name = it.next(); 
			Student student = dept.get(name); // key값으로 해시맵 Student 객체 얻어냄
			System.out.println("---------------------------");
	...
	private void processQuery() {
		while(true) {
			...
			if(name.equals("그만"))
				return; // 종료			
			Student student = dept.get(name); // 해시맵에서 이름을 key값으로 검색
			if(student == null) { // null값 이름이 없다면
